...
| **云端 AIStock** | 服务器 `http://<服务器IP>:9003` | 展示「策略扫描」「回写结果」页面 |
...
CLOUD_WRITEBACK_URL=http://<服务器IP>:9003/api/writeback-signals
...
| **云端 App** | `http://<服务器IP>:9003/` → 登录 → 「策略扫描」「回写结果」 | 完整扫描结果 + 候选回写信号（标注「模拟 dry-run」） |
...
```
扫描推送 成功 (HTTP 200) -> http://<服务器IP>:9003/api/strategy-scan
回写推送 成功 (HTTP 200) -> http://<服务器IP>:9003/api/writeback-signals
```

看到两个 `HTTP 200` 即推送成功；随后打开 `http://<服务器IP>:9003/` 登录，看「策略扫描」「回写结果」两个视图即为当天真实数据。
...
# 设置环境变量（token 须与云端一致）
export CLOUD_SCAN_URL=http://<服务器IP>:9003/api/strategy-scan
export CLOUD_SCAN_TOKEN=<你的token>
export CLOUD_WRITEBACK_URL=http://<服务器IP>:9003/api/writeback-signals
